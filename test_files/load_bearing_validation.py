...
system_message = (
    ...
    "\n3. Specify the building class you consider appropriate for the parking garage."
...
# Function to validate the agent's response
def validate_response(response):
    required_sections = [
        "description from the first column",
        "appropriate building class",
        "brief explanation for your choice"
    ]
    for section in required_sections:
        if section not in response.lower():
...
def choose_load_bearing_system(agent, theory_text, table_text):
    ...
    is_valid, validation_message = validate_response(response)
    if is_valid:
        return response
    else:
        return f"Invalid response: {validation_message}"
